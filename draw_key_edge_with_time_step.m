...
'ADD_Edge_betweenness_pic8_BA500_4t008nL1B1degreepredictionedgeEsasort.mat',...
...
zlabel('Cascade time steps in global breakdown')
...
%         % Periodic numbers
        if  exasorder==1
%         ylabel('Edge weight ratio w_{ij}k_{j}/s_{j}+w_{ij}k_{i}/s_{i}')
         ylabel('Edge weight ratio $e_{ij}$ of a removal edge $l_{ij}$ ','Interpreter','latex')

        else
%         ylabel({'Triggered the first stage neighbouring edge' ,'maximized α'})
        ylabel({'Triggered ${\alpha}_{\Gamma_{ij}}^{max}$ in neighbouring edges','edges of a removal edge $l_{ij}$ at first cascade time step'},'Interpreter','latex')
